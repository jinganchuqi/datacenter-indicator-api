...
class MetricsController extends AbstractController
{
    ...
    public function appDone(ResponseInterface $response): MessageInterface|\Psr\Http\Message\ResponseInterface
    {
        ...
        $funnelGauge = $registry->getOrRegisterGauge('app', 'done_count', 'app事件发生计数', ['country', 'bid', 'app_group_id', 'app_id', 'event']);
        $rateGauge = $registry->getOrRegisterGauge('app', 'done_rate', 'app事件发生率', ['country', 'bid', 'app_group_id', 'app_id', 'event']);
    ...
    JOIN conf.app on app.id=main.app_id
    WHERE event_name IN (
...
";
        ...
        foreach ($data as $row) {
            ...
            $this->setRate($rateGauge, $labels, $row, 'register', 'first_open', 'register');
    ...
    public function appDay7(ResponseInterface $response): MessageInterface|\Psr\Http\Message\ResponseInterface
    {
        ...
        // 2. 注册指标：标签只定义 [app_id, event]
        $registry = new CollectorRegistry(new InMemory(), false);
        $funnelGauge = $registry->getOrRegisterGauge('app', 'day7_count', 'app7日内事件发生计数', ['country', 'bid', 'app_group_id', 'app_id', 'event']);
